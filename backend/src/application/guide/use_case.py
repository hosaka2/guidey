...
class GuideUseCase:
        ...
        try:
            ...
            for coll in RAG_COLLECTIONS:
                if self._hybrid_client:
                    results.extend(self._hybrid_client.search(
                        collection=coll, query_embedding=query_embedding, query_text=goal,
                    ))
                else:
                    results.extend(self._rag_client.search(
                        collection=coll, query_embedding=query_embedding,
                    ))
            return results
        except Exception:
    ...
    def _build_prompt(
        self, trigger_word: str, goal: str = "", rag_results: list[RagResult] | None = None,
    ) -> str:
    ...
    async def analyze(
        ...
        instruction = await self._llm_client.analyze_image(
            image_bytes=image_bytes, system_prompt=system_prompt,
        )
